...
import { Users, Clock, AlertTriangle, CheckCircle, Pause, Play, StopCircle, Eye, RefreshCw } from 'lucide-react'
...
const ExamMonitorDashboard: React.FC<ExamMonitorDashboardProps> = ({
  ...
  const loadSessions = useCallback(async (showRefresh = false) => {
    try {
      ...
      // Calculate stats
      const newStats = (data.sessions || []).reduce((acc: SessionStats, session: ExamSessionWithDetails) => {
        acc.total++
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-semibold text-foreground">Exam Monitor</h2>
          <p className="text-secondary">Real-time monitoring of exam sessions</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <div className="bg-background rounded-lg border border-border p-4">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-sm font-medium text-secondary">Terminated</span>
          </div>
          <div className="text-2xl font-bold text-error">{stats.terminated}</div>
        </div>
      </div>
      ...
      <div className="bg-background rounded-lg border border-border overflow-hidden">
        ...
        {sessions.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-secondary">No exam sessions are currently running</p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead className="bg-background-secondary">
                <tr>
                  ...
                  <th className="text-left px-6 py-3 text-sm font-medium text-secondary">Started</th>
                  <th className="text-left px-6 py-3 text-sm font-medium text-secondary">Duration</th>
                  <th className="text-left px-6 py-3 text-sm font-medium text-secondary">Progress</th>
                  <th className="text-right px-6 py-3 text-sm font-medium text-secondary">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border">
                {sessions.map((session) => {
                  const timeElapsed = getTimeElapsed(session.started_at)
                  const examDuration = session.exams?.duration ? session.exams.duration * 60 : null
                  const timeRemaining = examDuration ? Math.max(0, examDuration - timeElapsed) : null
                  ...
                  const progressPercentage = totalQuestions > 0 ? Math.round((currentQuestionIndex / totalQuestions) * 100) : 0
                  ...
                  return (
                    <tr key={session.id} className="hover:bg-background-secondary">
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm text-foreground">{session.exams?.title}</div>
                      ...
                      <td className="px-6 py-4">
                        <div className={`text-sm font-mono ${isOvertime ? 'text-error' : 'text-foreground'}`}>
                          {formatTime(timeElapsed)}
                        </div>
                        {timeRemaining !== null && (
                          <div className={`text-xs ${isOvertime ? 'text-error' : 'text-secondary'}`}>
                            {isOvertime ? 'Overtime' : `${formatTime(timeRemaining)} left`}
                          </div>
                        )}
                      </td>
                      ...
                      <td className="px-6 py-4">
                        ...
                        <div className="text-xs text-secondary mt-1">
                          {progressPercentage}% complete ({currentQuestionIndex}/{totalQuestions})
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          {session.status === 'active' && (
                            <Button
                              onClick={() => handleSessionAction(session.id, 'terminate', 'Terminated by examiner')}
                              variant="ghost"
                              size="sm"
          ...
          <div className="bg-background rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6">
              {(() => {
                ...
                if (!session) return <p className="text-secondary">Session not found</p>
                ...
                return (
                  <div className="space-y-6">
                    ...
                    <div className="border-b border-border pb-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-foreground">{session.user_profiles?.email}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    {/* Exam Info */}
                    <div className="border-b border-border pb-4">
                      <h4 className="text-lg font-semibold text-foreground mb-3">Exam Details</h4>
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="text-sm text-secondary">Exam Title</label>
                          <p className="text-foreground font-medium">{session.exams?.title}</p>
                        </div>
                        <div>
                          <label className="text-sm text-secondary">Duration</label>
                          <p className="text-foreground">
                            {session.exams?.duration ? `${session.exams.duration} minutes` : 'No limit'}
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    {/* Session Progress */}
                    <div className="border-b border-border pb-4">
                      <h4 className="text-lg font-semibold text-foreground mb-3">Progress</h4>
                      <div className="space-y-4">
                        <div>
                          <div className="flex justify-between text-sm mb-1">
                            <span className="text-secondary">Questions Answered</span>
                            <span className="text-foreground">{currentQuestionIndex} of {totalQuestions}</span>
                          </div>
                          ...
                          <div className="text-center text-sm text-secondary mt-1">{progressPercentage}% Complete</div>
                        </div>
                        
                        <div className="grid grid-cols-3 gap-4">
                          <div className="text-center">
                            <div className="text-lg font-bold text-foreground">{formatTime(timeElapsed)}</div>
                            <div className="text-xs text-secondary">Time Elapsed</div>
                          </div>
                          {timeRemaining !== null && (
                            <div className="text-center">
                              <div className={`text-lg font-bold ${timeRemaining === 0 ? 'text-error' : 'text-foreground'}`}>
                          ...
                          <div className="text-center">
                            ...
                            <div className="text-xs text-secondary mt-1">Status</div>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    {/* Session Timestamps */}
                    <div>
                      <h4 className="text-lg font-semibold text-foreground mb-3">Timeline</h4>
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="text-sm text-secondary">Started At</label>
                          <p className="text-foreground">
                        ...
                        {session.completed_at && (
                          <div>
                            <label className="text-sm text-secondary">Completed At</label>
                            <p className="text-foreground">
