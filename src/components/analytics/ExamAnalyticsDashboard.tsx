...
import { TrendingUp, Users, Clock, Target, BarChart3, PieChart, RefreshCw } from 'lucide-react'
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-semibold text-foreground">Exam Analytics</h2>
          <p className="text-secondary">
            {examTitle ? `Analysis for "${examTitle}"` : 'Detailed performance analysis'}
          </p>
        </div>
        ...
        <div className="bg-background rounded-lg border border-border p-6">
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-medium text-foreground">
                  {formatPercentage((analytics.completedAttempts / analytics.totalAttempts) * 100)}
                </span>
              </div>
              <div className="w-full bg-background-secondary rounded-full h-2">
                <div
            ...
            <div className="pt-4 border-t border-border">
              <div className="text-sm text-secondary mb-1">Overall Performance</div>
              <div className="text-lg font-semibold text-foreground">
                {analytics.averageScore >= 80 ? 'Excellent' :
                 analytics.averageScore >= 70 ? 'Good' :
                 analytics.averageScore >= 60 ? 'Satisfactory' : 'Needs Improvement'}
              </div>
            </div>
          </div>
        </div>
      ...
      {analytics.questionAnalytics && analytics.questionAnalytics.length > 0 && (
        <div className="bg-background rounded-lg border border-border p-6">
          <h3 className="text-lg font-medium text-foreground mb-4">Question Analysis</h3>
      ...
      {analytics.totalAttempts === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-secondary">
            No exam attempts have been completed yet. Analytics will appear once students start taking the exam.
          </p>
        </div>
      )}
    </div>
