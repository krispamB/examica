    ...
    switch (action) {
      ...
      case 'terminate':
        ...
        if (['admin', 'examiner'].includes(userProfile.role)) {
          result = await examSessionService.terminateExamSession(
            sessionId,
            reason
          )
        } else {
      ...
      case 'update_metadata':
        // Only students can update metadata of their own sessions (after verification)
        if (
          userProfile.role === 'student' &&
          sessionResult.session.user_id === user.id
        ) {
          result = await examSessionService.updateSessionMetadata(
            sessionId,
            body.metadata
          )
        } else {
