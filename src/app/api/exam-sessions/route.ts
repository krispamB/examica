...
import type { StartExamRequest, ExamSessionFilters } from '@/lib/exam-sessions/service'
...
export async function GET(request: NextRequest) {
  try {
    ...
    const filters: ExamSessionFilters = {
      ...
      status: searchParams.get('status') as 'draft' | 'active' | 'completed' | 'cancelled' | undefined || undefined,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const examSessionService = createExamSessionService()
    const result = await examSessionService.startExamSession(user.id, startExamRequest)
