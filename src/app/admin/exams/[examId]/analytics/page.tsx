...
export default async function AdminExamAnalyticsPage({ params }: AdminExamAnalyticsPageProps) {
  const { examId } = params
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-foreground">Exam Analytics (Admin)</h1>
          ...
          <div className="flex items-center gap-6 text-sm">
            <div className="text-center">
              <div className="text-lg font-semibold text-foreground">{exam.question_count || 0}</div>
        ...
        <div className="bg-warning-light border border-warning/20 rounded-lg p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="text-sm font-medium text-warning">System-Wide Analytics</h3>
              <p className="text-sm text-warning mt-1">
                You are viewing system-wide analytics for this exam with full access to all performance data and student information.
              </p>
            </div>
          </div>
        </div>
...
export async function generateMetadata({ params }: AdminExamAnalyticsPageProps) {
  const { examId } = params
