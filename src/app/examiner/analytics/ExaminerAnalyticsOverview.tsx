...
import { BarChart3, TrendingUp, Users, FileText, Clock, Target, Eye } from 'lucide-react'
  ...
  if (error || !stats) {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-lg font-medium text-foreground mb-2">Analytics Error</h3>
        <p className="text-secondary mb-6">{error || 'Failed to load analytics'}</p>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <div className="bg-background rounded-lg border border-border p-4">
          ...
          <div className="text-xs text-secondary">{stats.activeExams} active</div>
        </div>
      ...
      <div className="bg-background rounded-lg border border-border">
        ...
        {stats.recentExams.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-secondary mb-6">Create your first exam to get started with analytics</p>
          ...
          <div className="divide-y divide-border">
            {stats.recentExams.map((exam) => (
              <div key={exam.id} className="px-6 py-4 hover:bg-background-secondary">
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-1">
                      <h4 className="text-base font-medium text-foreground">{exam.title}</h4>
                    ...
                    <div className="flex items-center gap-4 text-sm text-secondary">
                      ...
                      <span>{exam.duration ? `${exam.duration} min` : 'Unlimited time'}</span>
                  ...
                  <div className="flex items-center gap-2">
                    <div className="text-right mr-4">
                      <div className="text-sm font-medium text-foreground">0 attempts</div>
          ...
          <div className="bg-background rounded-lg border border-border p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-foreground font-medium">
                  {stats.totalExams > 0 ? Math.round((stats.activeExams / stats.totalExams) * 100) : 0}% active
                </span>
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-foreground font-medium">
                  {stats.totalExams > 0 
                    ? Math.round(stats.recentExams.reduce((sum, exam) => sum + (exam.question_count || 0), 0) / Math.min(stats.recentExams.length, 5))
                    : 0} per exam
                </span>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <Link href="/examiner/create" className="block">
                <div className="p-3 border border-border rounded-lg hover:bg-background-secondary transition-colors">
                  <div className="text-sm font-medium text-foreground">Create New Exam</div>
                  <div className="text-xs text-secondary">Start building a new examination</div>
                </div>
              </Link>
              
              <Link href="/examiner/exams" className="block">
                <div className="p-3 border border-border rounded-lg hover:bg-background-secondary transition-colors">
                  <div className="text-sm font-medium text-foreground">Manage Exams</div>
                  <div className="text-xs text-secondary">View and edit your existing exams</div>
                </div>
              </Link>
              
              <Link href="/admin/monitor" className="block">
                <div className="p-3 border border-border rounded-lg hover:bg-background-secondary transition-colors">
                  <div className="text-sm font-medium text-foreground">Monitor Live Sessions</div>
                  <div className="text-xs text-secondary">View active exam sessions</div>
                </div>
              </Link>
            </div>
          </div>
